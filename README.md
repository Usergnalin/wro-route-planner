...
- **The simulator reads the timeline, not `computeSteps()`** — the obvious source for a pose track is the steps the robot actually executes, but a `linetrace_junct` step carries only a junction COUNT with no distance, so dead reckoning loses the robot at the first junction trace and every pose after it is fiction. `RP.resolveTimeline()` has real coordinates for every move, so `RP.simPoseTrack()` walks that instead and is exact by construction.
- **Turns in place are swept, not skipped** — a long robot pivoting in a tight corner is a real way to hit something, so rotations are sampled at `RP.SIM_TURN_DEG` just as straights are sampled at `RP.SIM_STEP_MM`.
- **The footprint is a convex hull** — a body drawn as loose lines is a point cloud with no reliable ordering, so the hull is the smallest polygon containing everything drawn. Conservative in the right direction: it can flag a collision a concave body would have squeezed past, but it can never miss one.
- **Uncertainty is a box in MAT axes, and is off by default** — dead-reckoning drift grows an axis-aligned uncertainty box with distance driven, and each move that re-references something the robot can actually sense collapses it onto the direction that move does NOT measure: a wall align fixes distance to the wall but not position along it; a line trace fixes lateral but not along-track; a junction trace fixes both. Mat axes rather than robot-local, because that is the frame the corrections live in. `driftPerMm` defaults to **0** on purpose — the rate is a property of one robot on one surface, and inventing a plausible default would silently inflate every collision warning on a guess.
- **Certain and possible collisions are counted separately** — a hit is `certain` if the body collides at its true size, and merely possible if only the drift-inflated envelope reaches it. Collapsing the two would let a real collision hide among speculative ones.
- **A wall align trusts the geometry, exactly like a line trace** — it says what the robot DOES at the end of a leg; it is not a reason to move where the leg ends. Nothing is auto-constrained, nothing is auto-deleted, and switching a move's mode never writes to the sketch. Draw the end point where the turning centre should come to rest; if you want it held off a wall by the body's overhang, apply a **Distance to line** constraint yourself, in the sketcher, where you can see it and it can be solved against everything else you drew. (This replaced an automatic `point_line_distance` that fought the user's own constraints and shifted geometry the moment a move changed mode. Projects made before it keep their constraints as ordinary geometry — nothing touches them on load.)
- **The robot's extents are measured from the drawn body** — `RP.robotExtentsMm()` is what the collision sweep uses, and it is reported in Robot & Code so the drive axis can be checked against the drawing.
...
- [x] **Route segments** — **✂ Split** starts a new segment at the selected
      action, so one segment ends exactly where the next begins. Each segment
      header carries two independent switches: an eye (draw it or not) and a
      checkbox (put it in the generated code or not) — seeing a section you are
      not currently running is ordinary, so the two are not tied together.
      Double-click to rename, ✕ removes a boundary and merges into the segment
      above. Segments are named when you split (and renameable after), and
...
- [x] Action rows show each move's length, so a back-and-forth section reads
      `Straight · 400 mm` rather than three identical `Straight` rows
- [x] Clicking geometry in Route mode inserts it where it joins the route —
      bridging a gap lands it in the gap, extending the front lands it at the
      front, and sequential drawing still appends
...
- [x] 3 decimal places on every emitted distance and angle, and residual turns
      emitted rather than rounded away — so the angles the robot integrates
      always sum to the geometry they came from
...
- [x] Body reach ahead/behind/either side of the turning centre measured from the
      drawing and reported in Robot & Code — it is what the collision sweep uses
- [x] Obstacles: right-click mat geometry → Mark as obstacle. Rendered red,
      collected by `RP.obstacleSegments()` for the simulator
- [x] **Positional uncertainty** — set Drift (mm per mm driven) in Robot & Code and
      the swept body grows as the robot dead-reckons, shrinking again at each wall
      align (normal axis), line trace (lateral) and junction trace (both). Hits
      only the drifted envelope reaches are listed as "possible" in amber with a
      dashed envelope on canvas, separately counted from certain ones
- [x] **Collision simulation** (Route mode) — the robot body is swept along the
      whole route, turns in place included, and every contact with an obstacle
      is reported in a Simulation panel with how far into the route it happens
      and which obstacle it is. Click a warning to select the move it happened
      on; hover to highlight it on the canvas. Warnings only — nothing is
      blocked. Recomputed on edit (~15ms on a 100-action route), never on render
